...
def Constrained_least_square_filtering_temp():
    ...
    workflows = [
        ...
        {'title': '6) Optimization Algorithm',
            'description': 'Select an optimization algorithm to solve the constrained optimization problem. Common optimization techniques for constrained least square filtering include gradient descent, interior-point methods, and ADMM (Alternating Direction Method of Multipliers).'},
        ...
        {'title': '8) Solving the Optimization Problem',
            'description': 'Apply the chosen optimization algorithm to solve the constrained optimization problem. This will yield the filtered image that minimizes the objective function under the specified constraints.'},
...
def Order_statistics_filters_temp():
    ...
    workflows = [
        {'title': '1) Image Preprocessing',
            'description': 'Begin by acquiring the image you want to process. Ensure that the image is in the appropriate format and grayscale if necessary.'},
        ...
        {'title': '4) Order Statistics Selection',
            'description': 'Choose the specific order statistics operation you want to perform, such as median, minimum, maximum, or any other percentile (e.g., 10th percentile). The selection depends on the goal of the operation. For instance, the median is often used for noise reduction, while minimum and maximum may be used for morphological operations.'},
        ...
        {'title': '7) Border Handling', 'description': 'Handle the borders of the image appropriately. There are various methods for border handling, including zero-padding, mirror reflection, or periodic extension.'},
...
def Hormonic_Filter_temp():
    ...
    workflows = [
        ...
        {'title': '3) Harmonic Mean Calculation',
            'description': 'For each pixel in the image, compute the harmonic mean of the pixel values within the defined local neighborhood. The harmonic mean for a set of values (x1, x2, ..., xn) is calculated as n divided by the sum of the reciprocals of the values: H = n / (1/x1 + 1/x2 + ... + 1/xn).'},
...
def geometric_mean_temp():
    ...
    workflows = [
        ...
        {'title': '5) Local Window Scanning',
            'description': 'Slide the selected local window over the entire image. For each position, extract the pixel values within the window.'},
        ...
        {'title': '7) Output Image Update',
            'description': 'Replace the central pixel value within the local window with the computed geometric mean. This updates the output image.'},
...
def median_Filter_temp():
    ...
    workflows = [
        ...
        {'title': '2) Define the Neighborhood',
            'description': 'Choose the size of the neighborhood (typically a square or rectangular window) within which the median value will be calculated. The neighborhood size is a crucial parameter that affects the extent of noise reduction and the preservation of image details.'},
        ...
        {'title': '5) Repeat for All Pixels',
            'description': 'Iterate through all pixels in the image, applying the median filtering operation to each one.'},
...
def Salt_and_papper_noise_temp():
    ...
    workflows = [
        {'title': '1) Image Inspection and Preprocessing',
            'description': 'Start by examining the noisy image to understand the extent and nature of salt-and-pepper noise. Ensure that the image is properly preprocessed, such as converting it to grayscale if needed.'},
        ...
        {'title': '3) Visual Inspection and Quality Assessment',
            'description': 'Visually inspect the image and assess the quality of the noise reduction process. Take note of important image features that must be preserved during the denoising.'},
        {'title': '4) Thresholding',
            'description': 'Apply a threshold to the noisy image. Pixels with intensity values below the threshold are set to zero (black), while those above the threshold are set to the maximum intensity (white). This step may remove some of the noise, but it can also lead to information loss.'},
        ...
        {'title': '9) Quality Assessment and Validation',
            'description': 'Evaluate the denoised image using relevant quality metrics and visual inspection. Ensure that the salt-and-pepper noise is adequately reduced while preserving important image details.'},
...
def Gaussian_Noise_temp():
    ...
    workflows = [
        ...
        {'title': '2) Noise Characterization',
            'description': 'Analyze the noise in the image. Gaussian noise follows a normal distribution, so estimate the noise parameters, including the mean (usually close to zero) and standard deviation (sigma) to understand the noise characteristics.'},
        ...
        {'title': '4) Algorithm Selection',
            'description': 'Choose an appropriate noise reduction algorithm for Gaussian noise based on the noise characteristics. Common algorithms include: 1) Gaussian smoothing (convolution with a Gaussian kernel). 2) Wiener filtering. 3) Non-local means denoising. 4) Bilateral filtering. 4) Anisotropic diffusion. 5)  Total variation denoising.'},
        {'title': '5) Parameter Tuning',
            'description': 'If the chosen algorithm has adjustable parameters (e.g., kernel size, filter weights, or regularization parameters), experiment with different settings to find the optimal values. This can be done through cross-validation or other validation techniques.'},
